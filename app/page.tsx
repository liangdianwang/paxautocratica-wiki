...
export default function Home() {
  ...
  return <main className="shell home-shell">
    ...
    <section className="section home-video-section"><div className="container"><div className="section-head"><div><div className="section-kicker">WATCH THE GAME</div><h2>{home.hero.videoLabel || "Game video"}</h2></div><p>{homepageVideo?.title || "A verified game video selected for this site."}</p></div>{homepageVideo?.embed_url ? <div className="home-video-frame"><iframe data-home-video="true" src={homepageVideo.embed_url} title={homepageVideo.title || `${siteData.game?.name || "Game"} video`} loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div> : null}</div></section>
    ...
    <section className="section archive-section"><div className="container"><div className="section-head"><div><div className="section-kicker">THE GUIDE INDEX</div><h2>All published guides</h2></div><p>{visiblePages.length} focused guides are ready to use. Pages stay out of the public index until their evidence and answer are strong enough.</p></div><div className="archive-grid">{visiblePages.map((page: any) => <Link className="archive-card" href={`/${page.slug}`} key={page.slug}><div className="archive-card-top"><span className="status-dot ready" />VERIFIED GUIDE<span className="file-arrow" aria-hidden="true">↗</span></div><strong>{page.keyword}</strong><p>{page.direct_answer}</p></Link>)}</div><AdSlot id="home-2" /></div></section>
